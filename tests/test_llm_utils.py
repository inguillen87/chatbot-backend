...
class TestLLMUtils(unittest.TestCase):
    ...
    def test_update_summary_with_llm_extraction_intelligent_merge(self, mock_robust_chat):
        ...
        self.assertIn("CURRENT SUMMARY: '''El cliente Juan Pérez reportó un problema.'''", args[0]['message'])
        self.assertIn("NEW DATA (in JSON format): '''{", args[0]['message'])
        self.assertIn("\"tipo_problema\": \"Fuga de agua\"", args[0]['message'])
        self.assertIn("\"ubicacion_problema\": \"Baño principal\"", args[0]['message'])
