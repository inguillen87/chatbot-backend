    ...
    def test_contact_message_parsed_and_creates_ticket(self):
        ...
        chat_ctx.context_data = {
            CONTEXTO_MUNICIPIO: {
                ...
                'reclamo_flow_v2': {
                    'state': 'ESPERANDO_DATOS_CONTACTO',
                    'datos_reclamo': {
                        'categoria': 'Arbolado',
                        'descripcion': 'desc',
                        'ubicacion': 'Calle 1',
        ...
        viewer = MagicMock()
        with self.app.app_context(), patch('services.municipio_responder.CrearReclamoActionHandler') as mock_handler:
            inst = mock_handler.return_value
            inst.execute.return_value = {'success': True, 'message_to_user': 'ok', 'data': {}}
            summary = responder_municipio(
            ...
            self.assertIn('confirmá', summary['message_body'])
