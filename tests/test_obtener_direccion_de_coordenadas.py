...
class TestObtenerDireccionDeCoordenadas(unittest.TestCase):
    ...
    @patch('services.herramientas_municipio.cohere_client')
    @patch('services.herramientas_municipio.geocodificar_inversa_llm')
    def test_openai_success(self, mock_openai, mock_cohere, mock_parse):
        ...
        mock_cohere.generate.assert_not_called()
    ...
    @patch('services.herramientas_municipio.cohere_client')
    @patch('services.herramientas_municipio.geocodificar_inversa_llm')
    def test_cohere_fallback(self, mock_openai, mock_cohere, mock_parse, mock_requests_get):
        ...
        mock_cohere.generate.return_value = types.SimpleNamespace(
            generations=[types.SimpleNamespace(text='{"formatted_address": "San Martin 123, Mendoza"}')]
        )
        result = hm.obtener_direccion_de_coordenadas(-1, -1)
        assert result['calle'] == 'San Martin'
        mock_cohere.generate.assert_called_once()
