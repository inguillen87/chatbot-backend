...
class PymeMultimodalTest(unittest.TestCase):
    ...
    def test_image_payload_creates_assisted_intake_request_for_crm(self, mock_analyse, _mock_qdrant):
        ...
        assisted_tracking = assisted_request["public_follow_up"]["tracking"]
        self.assertEqual(assisted_tracking["code"], f"pc-{assisted_request['pedido_id']}")
        self.assertIn("/tracking/order/pc-", assisted_tracking["path"])
        self.assertTrue(assisted_tracking["token_required"])
        ...
        self.assertEqual(pedido.metadata_payload["public_follow_up"]["tracking"]["access"], "signed_link")
        ...
        self.assertEqual(
            intake_ticket.datos_extra["public_follow_up"]["tracking"]["token"],
            pedido.metadata_payload["public_follow_up"]["tracking"]["token"],
        )
    ...
    def test_image_payload_persists_manual_review_when_unreadable(self, _mock_analyse):
        ...
        self.assertEqual(pedido.metadata_payload["public_follow_up"]["tracking"]["code"], f"pc-{pedido.id}")
        self.assertTrue(pedido.metadata_payload["public_follow_up"]["tracking"]["token_required"])
        self.assertIn("token=", pedido.metadata_payload["public_follow_up"]["tracking"]["path"])
