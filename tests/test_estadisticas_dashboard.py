...
class EstadisticasDashboardRouteTest(unittest.TestCase):
    ...
    def test_dashboard_municipio_payload(self, mock_servicio, mock_build_stats):
        ...
        self.assertIn('heatmap_pulses', payload['map_layers'])
        ...
        self.assertIn('showcase', payload['metadata']['map']['heatmap'])
        self.assertTrue(payload['metadata']['map']['heatmap']['showcase']['animation']['enabled'])
        map_meta = payload['metadata']['map']['heatmap']
        self.assertIn('rendering', map_meta)
        self.assertTrue(map_meta['rendering'].get('supports_animations'))
        self.assertIn('hotspots', map_meta)
        self.assertIsInstance(map_meta['hotspots'], list)
        self.assertIn('analytics', payload['metadata'])
        self.assertIn('heatmap', payload['metadata']['analytics'])
    ...
    def test_dashboard_pyme_payload(self, mock_servicio, mock_metricas_cls, mock_build_stats):
        ...
        self.assertIn('heatmap_cells', payload)
        self.assertIn('heatmap_cells_geojson', payload)
        pyme_map_meta = payload['metadata']['map']['heatmap']
        self.assertIn('rendering', pyme_map_meta)
        self.assertFalse(pyme_map_meta['rendering'].get('supports_heatmap'))
