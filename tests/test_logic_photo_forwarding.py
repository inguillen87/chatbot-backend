...
class LogicPhotoForwardingTest(unittest.TestCase):
    ...
    @patch('services.logic.db.session')
    def test_foto_url_local_path(self, mock_db_session, mock_file, mock_get, mock_interpretar, mock_responder):
        app = Flask(__name__)
        app.config['TWILIO_ACCOUNT_SID'] = 'sid'
        app.config['TWILIO_AUTH_TOKEN'] = 'token'
    ...
    @patch('builtins.open', new_callable=mock_open, read_data=b'data')
    @patch('services.logic.db.session')
    def test_foto_url_local_path_public_url(self, mock_db_session, mock_file, mock_get, mock_interpretar, mock_responder):
        app = Flask(__name__)
        app.config['TWILIO_ACCOUNT_SID'] = 'sid'
        app.config['TWILIO_AUTH_TOKEN'] = 'token'
        ...
        mock_get.assert_not_called()
        public_url = 'https://cdn.example.com/static/uploads/img.jpg'
        self.assertEqual(uploaded.get("public_url"), public_url)
        _, kwargs = mock_responder.call_args
        self.assertEqual(kwargs.get("foto_url"), public_url)
