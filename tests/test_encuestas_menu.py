...
# Alias module to mirror production import style used in tests
municipio_responder = sys.modules[__name__]


def _base_context(tenant_id: int = 1, extra_config: Optional[dict] = None) -> dict:
    """Build a minimal chat context for encuestas tests."""

    municipio_config = {"encuestas": {"enabled": True}}
    if extra_config:
        municipio_config.update(extra_config)

    dummy_user = type("AnonUser", (), {"municipio_id": tenant_id, "id": tenant_id})()
...
from tests.test_encuestas_module import _create_active_encuesta
import math
    ...
    def handle_direccion(self, user_input, payload):
        ...
        if payload.get("es_ubicacion") and payload.get("ubicacion_usuario"):
            ...
            if lat_value is not None and lon_value is not None:
                from services.herramientas_municipio import obtener_direccion_de_coordenadas
...
def test_encuesta_share_payload_adds_whatsapp_media_pre_message(client):
    with client.application.app_context():
        encuesta, slug, _user = _create_active_encuesta(tenant_id=41)
        context = _base_context(tenant_id=encuesta.tenant_id or 41)
        context["channel"] = "whatsapp"
        share_media_url = "https://cdn.chatboc.test/encuesta-banner.png"
        share_message = "Participá en encuesta test"
    ...
    context["chat_db_context_data"] = {
        municipio_responder.CONTEXTO_MUNICIPIO: {
            "encuestas_menu_surveys": [
                    {
                        "slug": slug,
                        "titulo": encuesta.titulo or "Encuesta ciudadana",
                        "share_message": share_message,
                        "share_url": f"https://encuestas.test/e/{slug}",
                        "share_short_url": f"https://short.test/e/{slug.rsplit('-', 1)[-1]}",
                        "share_image_url": share_media_url,
                        "share_media_urls": [share_media_url],
                    }
                ],
