...
def test_get_summary_returns_metrics(client):
    ...
    primer_canal = resumen["canales"][0]
    assert primer_canal["respuestas"] == primer_canal["conteo"] == primer_canal["value"]
    ...
    utm_items = resumen["utm"]
    assert utm_items
    first_utm = utm_items[0]
    assert first_utm["respuestas"] == first_utm["conteo"] == first_utm["value"]
    assert first_utm["fuente"] == first_utm["utm_source"]
    assert first_utm["campania"] == first_utm["utm_campaign"]
