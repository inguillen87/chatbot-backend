...
except Exception:  # pragma: no cover - Flask might be absent
    Flask = None
...
class GoogleLoginRouteTests(unittest.TestCase):
    if Flask is None:
        raise unittest.SkipTest("Flask no disponible")
    ...
    def test_google_login_route(self, mock_login):
        mock_login.return_value = SimpleNamespace(id=3, token='tok', name='Foo', email='foo@example.com')
        resp = self.client.post('/auth/google-login', json={'id_token': 'abc'})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.get_json()['email'], 'foo@example.com')
