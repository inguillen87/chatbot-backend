...
class TestParseDireccionFallback(unittest.TestCase):
    ...
    @patch.object(hm, 'geocode_address')
    @patch.object(hm, 'parse_direccion_completa')
    def test_validar_y_formatear_direccion_llm_fallback_fuera_de_limites(
        self, mock_parse, mock_geo, mock_resolver
    ):
        mock_resolver.return_value.resolve.return_value = None
        mock_resolver.return_value._within_bounds.return_value = False
        mock_parse.return_value = {
            "calle": "Sarmiento",
            "numero": "100",
            "localidad": "Junín",
        ...
        mock_geo.return_value = {
            "lat": -34.0,
            "lng": -69.0,
            "display_name": "Sarmiento 100, Otro Lugar",  # outside bounds
        }
        cfg = {"ciudad": "Junín", "provincia": "Mendoza", "pais": "AR", "bounds": (-68.6, -33.1, -68.4, -32.9)}
        res = hm.validar_y_formatear_direccion("Sarmiento 100", cfg)
        ...
        mock_parse.assert_called_once()
        mock_geo.assert_called_once()
        mock_resolver.return_value._within_bounds.assert_called_once()
