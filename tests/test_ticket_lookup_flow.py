...
def run_turn(message, owner_user, state=None, numero=None):
    """Helper function to simulate a turn in the conversation for ticket lookup."""
    session_id = "ticket_session"
    ctx = db.session.get(ChatSessionContext, session_id)
    if not ctx:
    ...
    ctx_after = db.session.get(ChatSessionContext, session_id).context_data[CONTEXTO_MUNICIPIO]
    return types.SimpleNamespace(response=resp, ctx=ctx_after)

def test_consulta_flow_direct_number(init_database, owner_user):
    """
...
def test_ticket_summary_has_basic_links(monkeypatch, init_database, owner_user):
    """
...
def test_invalid_ticket_pin(monkeypatch, init_database, owner_user):
    """
