...
def enabled():
    value = current_app.config.get('CRM_TASKS_ENABLED', os.getenv('CRM_TASKS_ENABLED', 'false'))
...
def capabilities(actor, slug):
    available = ready()
    body = {'contract_version': CONTRACT, 'tenant_slug': slug, 'available': available}
    if available:
