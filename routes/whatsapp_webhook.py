...
def handle_pyme_welcome(pyme_id: int, normalized_phone: str, profile_name: str, pyme_context: dict):
    ...
    # This would use a real WhatsApp sending service
    print(f"SENDING TO {normalized_phone}: {welcome_text}")

    # In a real app, this would be a background task
    def send_delayed_menu():
        with current_app.app_context():
            menu_payload = get_pyme_menu_payload(pyme_context)
            # This would format and send the menu
            print(f"SENDING DELAYED MENU TO {normalized_phone}: {menu_payload}")
    ...
    if client_user.tipo_chat == "pyme":
        pyme_context = session_context_db_entry.context_data.get("contexto_pyme_v2", {})
        if not pyme_context:
            from services.herramientas_pyme import get_static_pyme_data
            pyme_context = get_static_pyme_data(empresa_id)
            pyme_context["nombre_cliente"] = post_vars.get("ProfileName")
            session_context_db_entry.context_data["contexto_pyme_v2"] = pyme_context

        if should_trigger_welcome(incoming_text, session_context_db_entry.context_data):
            return handle_pyme_welcome(empresa_id, from_number_cleaned, post_vars.get("ProfileName"), pyme_context)

        # This part will be completed later to call responder_pyme_v4
        bot_response_dict = {"message_body": "PYME logic to be implemented."}

    elif should_trigger_welcome and not is_rate_limited:
