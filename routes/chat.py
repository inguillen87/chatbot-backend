...
from models import User, Rubro, Conversacion, db, ChatSessionContext
...
def _parse_request(tipo_chat_fijo: str | None = None):
    def _normalizar_tipo_chat(valor: str | None) -> str | None:
        ...
        sinonimos = {
            "pymes": "pyme", "pyme": "pyme",
            "municipios": "municipio", "municipio": "municipio", "muni": "municipio",
        }
    ...
    try:
        ...
        if location and not (isinstance(location, dict) and "lat" in location and "lon" in location):
        ...
        return (None, None, None, None, None, None, None, None, None, jsonify({"error": str(e)}))
    except Exception as e:
        current_app.logger.error(f"Error inesperado al parsear /ask: {e}")
        return (None, None, None, None, None, None, None, None, None, jsonify({"error": "Formato JSON inválido"}))
...
def _procesar_chat(tipo_chat_fijo: str | None = None, current_user=None, owner_user=None, anon_id: str | None = None):
    channel = "web"
    ...
    if not chat_context_obj:
        ...
        chat_context_obj = ChatSessionContext(chat_session_id=chat_session_id_header, user_id=getattr(actor_principal, 'id', None), anon_id=anon_id if not actor_principal else None, context_data={})
    ...
    if 'audio_file' in request.files:
        ...
        if audio_file.filename != '':
            ...
            os.remove(temp_path)
            if not pregunta:
                return jsonify({"message_body": "Lo siento, no pude entender lo que dijiste en el audio. ¿Podrías intentarlo de nuevo o escribir tu consulta?", "message_type": "text", "fuente": "audio_transcription_failed"}), 400
        ...
        chat_context_obj.context_data.pop('source_is_audio', None)
        try:
            (pregunta, contexto_previo, tipo_chat, rubro_id, rubro_clave, attachment_info, location, ticket_id, tipo_ticket, error_response) = _parse_request(tipo_chat_fijo)
            if error_response:
        ...
        current_app.logger.debug("Parsed request data", extra={"pregunta": pregunta, "tipo_chat": tipo_chat, "rubro_id": rubro_id, "rubro_clave": rubro_clave, "attachment_info": attachment_info, "location": location, "ticket_id": ticket_id, "tipo_ticket": tipo_ticket})
    if ticket_id and tipo_ticket and pregunta:
        ...
        if ticket and ticket.estado in ["esperando_agente_en_vivo", "en_proceso", "en_vivo"]:
            comentario_data = {"comentario": pregunta, "user_id": getattr(current_user, "id", None), "anon_id": anon_id if not current_user else None, "es_admin": False}
            if attachment_info:
            ...
            nuevo_comentario = servicio_tickets.crear_comentario(ticket_id=ticket_id, tipo_ticket=tipo_ticket, comentario_data=comentario_data)
            if nuevo_comentario:
                db.session.commit()
                room_name = f"ticket_{tipo_ticket}_{ticket_id}"
                socketio.emit('new_chat_message', {'ticket_id': ticket_id, 'message': nuevo_comentario.to_dict()}, room=room_name)
    ...
    try:
        ...
        if is_anonymous:
            ...
            if not session_expired:
                ...
                if message_count_this_session >= max_messages:
                    return jsonify({"error": "Alcanzaste el límite de mensajes para usuarios invitados.", "respuesta": "Alcanzaste el límite de mensajes para usuarios invitados. Para continuar, por favor inicia sesión o regístrate.", "botones": [{"texto": "Iniciar Sesión", "action": "login"}, {"texto": "Registrarme Gratis", "action": "register"}]}), 403
        else:
        ...
        if owner_del_bot:
            ...
            if limite is not None and owner_del_bot.preguntas_usadas >= limite:
                return jsonify({"error": f"El bot ha alcanzado el límite de preguntas de su plan ({limite})."}), 403
        ...
        if last_msg == pregunta and last_time_str:
            try:
                ...
                pass
        resultado = responder_chatboc(pregunta=pregunta, owner_user=owner_del_bot, viewer_user=viewer_obj, rubro_obj=rubro_obj_global, rubro_nombre_frontend=rubro_clave, tipo_chat=tipo_chat, contexto_previo=contexto_previo, anon_id=anon_id, chat_session_uuid=chat_session_id_header, chat_db_context=chat_context_obj, channel="web", attachment_info=attachment_info, location=location, user_data={"name": actor_principal.name, "email": actor_principal.email, "telefono": actor_principal.telefono} if actor_principal else None)
        if chat_context_obj:
            from services.municipio_responder import serializar_enum, CONTEXTO_MUNICIPIO
        ...
        current_app.logger.info(f"[RUBROS] Rubro efectivo: '{nombre_rubro_log}' (ID: {getattr(rubro_obj_global, 'id', 'N/A')}), esPublico={es_publico}")
        if owner_del_bot:
        ...
        if channel == "web" and chat_session_id_header:
            ...
            current_app.logger.debug("Emitting socket message", extra={"room": chat_session_id_header, "payload": resultado})
            current_app.logger.info(f"Emitted socket event 'message' to room {chat_session_id_header}")
        current_app.logger.debug("Returning HTTP response", extra={"payload": resultado})
        return jsonify(resultado), 200
    except Exception as e:
        db.session.rollback()
        error_details = {"pregunta": pregunta if 'pregunta' in locals() else 'N/A', "tipo_chat": tipo_chat if 'tipo_chat' in locals() else 'N/A', "rubro_id": rubro_id if 'rubro_id' in locals() else 'N/A', "rubro_clave": rubro_clave if 'rubro_clave' in locals() else 'N/A', "actor_principal_id": actor_principal.id if 'actor_principal' in locals() and actor_principal else 'N/A', "owner_del_bot_id": owner_del_bot.id if 'owner_del_bot' in locals() and owner_del_bot else 'N/A', "viewer_obj_id": viewer_obj.id if 'viewer_obj' in locals() and viewer_obj else 'N/A', "anon_id": anon_id if 'anon_id' in locals() else 'N/A', "archivo_adjunto_id": archivo_adjunto_id if 'archivo_adjunto_id' in locals() else 'N/A', "uploaded_file_info": uploaded_file_info if 'uploaded_file_info' in locals() else 'N/A', "session_chat_id": session_chat_id if 'session_chat_id' in locals() else 'N/A'}
        current_app.logger.error(f"❌ Error crítico en _procesar_chat. Details: {error_details}. Exception: {e}", exc_info=True)
