...
def _remove_item_from_current_cart(item_id):
    tenant, owner = _resolve_owner_and_seed()
    if not tenant or not owner:
        return _tenant_missing_response()
    ...
    cart_item = cart.items.filter(
        or_(
            MarketCartItem.product_id == item_id,
            MarketCartItem.id == item_id,
        )
    ).first()
    if cart_item:
        db.session.delete(cart_item)
...
def eliminar():
    ...
    payload = request.get_json(silent=True) or {}
    item_id = payload.get('catalogo_item_id') or payload.get('item_id') or payload.get('product_id') or payload.get('id') or payload.get('producto_id')
    return _remove_item_from_current_cart(item_id)


@carrito_bp.route('/<item_id>', methods=['DELETE', 'OPTIONS'])
@cross_origin(**_cors_kwargs(["DELETE", "OPTIONS"]))
@require_tenant
