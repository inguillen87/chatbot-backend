...
from routes.carrito import agregar, carrito_root, eliminar, eliminar_item, vaciar, actualizar
...
def _bind_tenant_slug_for_alias(tenant_slug: str) -> None:
    if not tenant_slug:
        return
    try:
...
def carrito_alias_with_slug(tenant_slug: str):
    ...
    _bind_tenant_slug_for_alias(tenant_slug)

    return carrito_root()


@api_aliases_bp.route("/carrito/<item_id>", methods=["DELETE", "OPTIONS"], strict_slashes=False)
def carrito_alias_eliminar_item(item_id: str):
    return eliminar_item(item_id)


@api_aliases_bp.route(
    "/<tenant_slug>/carrito/<item_id>", methods=["DELETE", "OPTIONS"], strict_slashes=False
)
def carrito_alias_eliminar_item_with_slug(tenant_slug: str, item_id: str):
    """Alias for modern marketplace clients that remove cart lines by path."""
...
def root_carrito_alias_eliminar_item_with_slug(tenant_slug: str, item_id: str):
    """Public alias to remove a cart item from /<slug>/carrito/<item_id>."""
