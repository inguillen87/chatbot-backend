"""Small thread-safe proxy for optional SDKs that are expensive to import.

The proxy deliberately behaves like a module attribute container: existing
``unittest.mock.patch("module.sdk.Client")`` seams keep working, while the real
package is imported only when one of its attributes is first used.
"""
