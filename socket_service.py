...
@socketio.on('call_agent')
def on_call_agent(data):
    """Notify available agents that a user is requesting a video call."""
    room = data.get('room')
    if room:
        emit('incoming_call', {'sid': request.sid}, room=room)


@socketio.on('webrtc_offer')
def on_webrtc_offer(data):
    """Forward a WebRTC offer to the target room."""
    room = data.get('room')
    if room:
        emit('webrtc_offer', {'sdp': data.get('sdp'), 'sid': request.sid}, room=room)


@socketio.on('webrtc_answer')
def on_webrtc_answer(data):
    """Forward a WebRTC answer to the target room."""
    room = data.get('room')
    if room:
        emit('webrtc_answer', {'sdp': data.get('sdp'), 'sid': request.sid}, room=room)
...
def on_webrtc_ice_candidate(data):
    ...
    room = data.get('room')
    if room:
        emit('webrtc_ice_candidate', {'candidate': data.get('candidate'), 'sid': request.sid}, room=room)


@socketio.on('end_call')
def on_end_call(data):
    """Propagate a call termination signal to the room."""
    room = data.get('room')
    if room:
        emit('end_call', {'sid': request.sid}, room=room)
