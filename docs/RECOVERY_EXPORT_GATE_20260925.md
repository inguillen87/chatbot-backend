...
La primera ejecuci?n nativa encontr? que el binario PostgreSQL del contenedor se expone mediante un enlace del sistema. Se resuelve el ejecutable expl?cito a su ruta real antes de validarlo; las restricciones de enlaces del archivo y su destino no se relajaron. Tambi?n se corrigi? el cierre expl?cito de objetos binarios en el fixture de psycopg2. Las pruebas de corte ya existentes aprobaron 42 casos sin modificaciones.
