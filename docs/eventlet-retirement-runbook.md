...
- `GUNICORN_THREADS` defaults to `32`. Explicit integer overrides are preserved
  within the bounded `16..64` range; lower or higher values are clamped to the
  nearest limit. This is bounded per-instance concurrency, not unlimited scale.
