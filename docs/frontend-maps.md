...
- `GET /estadisticas/mapa_calor/datos` devuelve puntos geojson para construir un mapa de calor de tickets. Admite parámetros opcionales como `tipo_ticket`, `municipio_id`, `rubro_id`, `fecha_inicio`, `fecha_fin`, `categoria` y `estado`. El payload expone `heatmap_geojson` y `heatmap_google` (para compatibilidad), además de `map_layers.heatmap` y `map_config` que indican el formato preferido según el proveedor disponible.
