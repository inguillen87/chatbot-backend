...
import re
import logging
from typing import Optional, Tuple, List # Asegúrate de tener List aquí también
...
def parse_precio_flexible(texto_precio_input: Optional[str | float | int]) -> Tuple[Optional[str], Optional[float], Optional[str]]:
    ...
    if re.search(r"(?i)\bUSD\b|U\$S", numero_para_procesar):
        ...
        moneda_detectada = "ARS"
        numero_para_procesar = re.sub(r"(?i)\bARS\b", "", numero_para_procesar, flags=re.IGNORECASE).strip()
    elif re.search(r"(?i)\bEUR\b", numero_para_procesar):
        moneda_detectada = "EUR"
        numero_para_procesar = re.sub(r"(?i)\bEUR\b", "", numero_para_procesar, flags=re.IGNORECASE).strip()
    
    if "€" in numero_para_procesar:
        moneda_detectada = "EUR"
        numero_para_procesar = numero_para_procesar.replace("€", "").strip()
    elif "$" in numero_para_procesar:
        if moneda_detectada == "ARS": 
    ...
    if ',' in numero_normalizado and '.' in numero_normalizado:
        ...
        if len(partes_coma) > 1 and len(partes_coma[-1]) <= 2 and partes_coma[-1].isdigit():
    ...
    try:
        if not numero_final_para_float_str or not re.search(r"\d", numero_final_para_float_str):
            raise ValueError("String numérico vacío o sin dígitos")
...
def extraer_unidades_y_tipos_precio(texto_linea: str, pyme_rubro_nombre: str = "generico") -> tuple[Optional[str], Optional[str]]:
    ...
    texto_linea_temp = texto_linea_lower
    ...
    for tipo, keywords in tipos_precio_keywords.items():
        for kw in keywords:
            if re.search(r'\b' + re.escape(kw.replace("?", "\\w?")) + r'\b', texto_linea_lower): 
                tipo_precio = tipo; break
