...
def buscar_catalogo_qdrant(user_id: int, pregunta: str, limite: int = 3, score_min: float = 0.65) -> List[qdrant_models.ScoredPoint]: # Ajustado score_min a 0.65
    ...
    try:
        ...
        vector_pregunta_lista = embed_textos([pregunta_limpia], input_type="search_query") # IMPORTANTE: search_query
...
def armar_respuesta_legible(resultados_qdrant: List[qdrant_models.ScoredPoint]) -> str:
    ...
    for hit in resultados_qdrant:
        ...
        parts = [f"Nombre: {n}"] # Siempre mostrar nombre
        if c: parts.append(f"Categoría: {c}")
        ...
        # Añadir descripción solo si es diferente al nombre y tiene contenido significativo
        desc_limpia = limpiar_texto_base(d)
        if desc_limpia and desc_limpia != limpiar_texto_base(n): 
            parts.append(f"Descripción: {(d[:100] + '...') if len(d) > 100 else d}")
