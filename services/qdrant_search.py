...
# O si usas una versión muy nueva de qdrant-client y lo anterior da error, podrías necesitar:
# from qdrant_client import QdrantClient, models as qdrant_models
# O importar específicamente: from qdrant_client.http.models import Filter, FieldCondition, MatchValue

from .utils import limpiar_texto_base 

logger = logging.getLogger(__name__)

def buscar_catalogo_qdrant(user_id: int, pregunta: str, limite: int = 3, score_min: float = 0.68) -> List[qdrant_models.ScoredPoint]:
...
def armar_respuesta_legible(resultados_qdrant: List[qdrant_models.ScoredPoint]) -> str:
    ...
    for hit_idx, hit in enumerate(resultados_qdrant):
        ...
        elif payload.get("precio_float") is not None: 
