...
def _extraer_cantidades_con_llm(pregunta_cliente: str, productos_disponibles_raw: list) -> list:
    ...
    for p in productos_disponibles_raw:
        ...
        if sku and sku != nombre_completo and sku != "N/A":
            ...
            # --- CORRECCIÓN CRÍTICA DE LA F-STRING: USAR COMILLAS TRIPLES ---
    ...
    try:
        ...
        if pedio_todos and productos_disponibles_raw:
            cantidad_general = 1
            # Buscar cantidad en la pregunta para aplicar a "cada variedad"
            ...
            for item_llm in parsed_json:
                ...
                for p_raw in productos_disponibles_raw:
                    ...
                    if identificador == nombre_completo or identificador == sku:
                        ...
                        break
                    # Match flexible si el LLM no dio el nombre exacto
...
class FollowUpHandler(BaseHandler):
    def handle(self, pregunta: str) -> dict | None:
        ...
        elif 'confirmando_pedido_final_paso_2' in contexto_pyme: # Nuevo estado para la creación final
            productos_a_confirmar = contexto_pyme.pop('productos_a_confirmar_en_paso_2') # Trae los productos del contexto
            monto_total_final = contexto_pyme.pop('monto_total_final_en_paso_2', 0.0)
            
            # Intentar extraer contacto del usuario si no se hizo antes o si lo da aquí
            nombre = None
            email = None
            telefono = None
...
Tienes razón. Revisa la línea `elif p.get('descripcion'):` debe estar indentada en un bloque `if/else`.
```python
# services/pymes.py
...
def _extraer_cantidades_con_llm(pregunta_cliente: str, productos_disponibles_raw: list) -> list:
    ...
    for p in productos_disponibles_raw:
        ...
        if sku and sku != nombre_completo and sku != "N/A":
            ...
            # --- CORRECCIÓN CRÍTICA DE INDENTACIÓN Y F-STRING: USAR COMILLAS TRIPLES ---
