    ...
    def _run(self) -> None:
        while not self._stop.wait(self.interval):
            try:
                ...
                self._lost.set()
                logger.exception(
                    "[WHATSAPP_WORKER] Inbound lease heartbeat failed turn_id=%s",
                    self.claim.turn_id,
                )
                return
...
def process_whatsapp_inbound_claim(
    ...
    except (WhatsAppWorkerScopeError, HTTPException) as exc:
        heartbeat.stop()
        error_code = (
            str(exc)
            if isinstance(exc, WhatsAppWorkerScopeError)
            else f"webhook_replay_http_{getattr(exc, 'code', 400)}"
        )
...
def process_whatsapp_inbound_stream(
    *,
    ...
    """Drain a bounded number of FIFO turns from one stream or the DB queue."""
...
def run_whatsapp_inbound_payload_scrub(*, limit: Optional[int] = None) -> dict[str, Any]:
    """Run one bounded retention batch from explicit deployment policy."""
    ...
    report = scrub_expired_whatsapp_inbound_payloads(
        dead_retention_hours=current_app.config.get(
            "WHATSAPP_INBOUND_DEAD_PAYLOAD_RETENTION_HOURS",
            72,
        ),
        legal_hold=configured_hold,
        limit=(
            limit
    ...
    logger.info(
        "[WHATSAPP_RETENTION] status=%s eligible=%s scrubbed=%s remaining=%s legal_hold=%s",
        report["status"],
        report["eligible"],
        report["scrubbed"],
        report["remaining_eligible"],
        report["legal_hold"],
    )
    return report
    ...
    with app.app_context():
        ...
        while not shutdown.is_set():
            ...
            try:
                ...
                db.session.rollback()
                db.session.remove()
                logger.exception("[WHATSAPP_WORKER] Durable poll cycle failed")
                did_work = False
...
def main() -> int:
    ...
    os.environ.setdefault("CHATBOC_PROCESS_ROLE", "whatsapp-durable-worker")
    ...
    run_whatsapp_durable_worker(app, once=args.once, stop_event=shutdown)
    return 0
