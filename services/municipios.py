        ...
        if estado_confirm == ConversationState.ESPERANDO_CONFIRMACION_RECLAMO:
            ...
            if self.context.get("anon_id") and not self.context.get("cliente_id"):
                ...
                if anon_tickets_count >= max_tickets_anon:
                    memoria.clear()
                    return {"respuesta": "Llegaste al límite de reclamos para usuarios no registrados. Para continuar, por favor registrate o iniciá sesión.", "botones": [{"texto": "Registrarme"}, {"texto": "Iniciar Sesión"}]}
            ...
            if accion == "confirmar_reclamo_final" or (accion != "editar_reclamo_datos" and any(kw in accion for kw in PALABRAS_CLAVE_CONFIRMACION)):
                ...
                try:
                    ...
                    return {"respuesta": "¡Upa! Hubo un problema al registrar tu reclamo. ¿Querés intentar de nuevo o preferís que te ayude con otra cosa?", "botones": [{"texto": "Intentar de nuevo"}, {"texto": "Cancelar"}]}
                ...
                resumen = self.build_detalles_memoria(memoria)
                return {"respuesta": f"No entendí tu respuesta. Por favor, confirmá si los datos son correctos:\n\n{resumen}\n\n¿Confirmamos el reclamo?", "botones": [{"texto": "Sí, confirmar"}, {"texto": "No, editar datos"}]}
