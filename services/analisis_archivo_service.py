...
def tarea_analizar_contenido_archivo(self, archivo_adjunto_id: int):
    ...
    try:
        ...
        # Determinar contexto municipal
        es_contexto_municipal = False
        if user_obj:
            if user_obj.tipo_chat == "municipio":
                es_contexto_municipal = True
            elif user_obj.rubro and es_rubro_publico(user_obj.rubro.clave if hasattr(user_obj.rubro, 'clave') else str(user_obj.rubro)): # Asegurar que se usa la clave del rubro si es un objeto
                es_contexto_municipal = True
...
def _realizar_analisis_ocr_simple(session, analisis_archivo_id: int):
    ...
    try:
        ...
        from services.google_vision_service import analyze_image_from_content as vision_analyze

        vision_data = vision_analyze(image_content)

        if vision_data.get("error"):
            raise ValueError(f"Error de Vision API en OCR simple: {vision_data['error']}")

        texto_extraido_ocr = ""
        if vision_data.get("text_annotations"):
            texto_extraido_ocr = vision_data["text_annotations"][0].get("description", "").strip()

        analisis_archivo.texto_extraido = texto_extraido_ocr if texto_extraido_ocr else None
        analisis_archivo.tipo_analisis = "vision_ocr_simple"
        analisis_archivo.error_analisis = None
        logger.info(f"OCR simple completado para Archivo ID: {archivo_adjunto.id}. Texto: '{texto_extraido_ocr[:100] if texto_extraido_ocr else 'N/A'}'")

        # No marcamos como completado aquí, se hace en la tarea principal
        # analisis_archivo.estado_analisis = "completado"
    except Exception as e:
        logger.error(f"Error durante OCR simple de imagen ID {archivo_adjunto.id}: {e}", exc_info=True)
        analisis_archivo.estado_analisis = "error"
        analisis_archivo.error_analisis = f"Error en OCR simple: {str(e)}"
        analisis_archivo.tipo_analisis = "vision_ocr_simple_error"

def analizar_imagen_con_vision_ocr_service(session, analisis_archivo_id: int):
    # Esta función ahora es un alias o se reemplaza por _realizar_analisis_ocr_simple
    # o se elimina si la lógica principal de la tarea la maneja directamente.
    # Por ahora, la mantendremos y haremos que llame a la nueva función de OCR simple
    # para no romper la estructura si se llamaba desde otro lado (aunque parece que no).
    logger.warning("analizar_imagen_con_vision_ocr_service está obsoleta, usar lógica directa en tarea o _realizar_analisis_ocr_simple.")
    _realizar_analisis_ocr_simple(session, analisis_archivo_id)
