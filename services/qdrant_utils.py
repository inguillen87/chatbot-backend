...
qdrant_client_instance: Optional[QdrantClient] = None # Tipado

def get_qdrant_client() -> Optional[QdrantClient]: # Tipado
...
def verificar_y_crear_coleccion_qdrant(collection_name: str, vector_size: int, distance_metric: str = "Cosine") -> bool:
    ...
    if not client:
        logger.error(f"[QDRANT UTILS] No se pudo obtener cliente Qdrant para '{collection_name}'.")
        return False
    try:
        ...
        logger.error(f"❌ [QDRANT UTILS] Error al verificar/crear colección '{collection_name}': {e}", exc_info=True)
        return False