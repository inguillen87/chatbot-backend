...
class CrearReclamoAction(BaseActionHandler):
    def execute(self, action_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not all([categoria, descripcion, ubicacion_texto]): # Basic validation
            missing = [f for f,v in {"categoría":categoria, "descripción":descripcion, "ubicación":ubicacion_texto}.items() if not v]
            return {
                "success": False,
                "message_to_user": f"Faltan datos para crear el reclamo: {', '.join(missing)}. Por favor, intenta de nuevo.",
                "pedir_info": missing[0] if missing else "datos_reclamo_faltantes", # Signal to ask for the first missing field
