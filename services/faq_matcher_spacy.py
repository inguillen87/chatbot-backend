...
def buscar_en_faq_spacy(pregunta_usuario: str, rubro_id: int, threshold: float = 0.80) -> Optional[QA]:
    ...
    if NLP_SPACY_FAQ is None: logger.error("[FAQ] Imposible buscar: modelo spaCy no está cargado."); return None
    if not pregunta_usuario or not isinstance(pregunta_usuario, str) or not pregunta_usuario.strip(): logger.warning("[FAQ] Pregunta de usuario vacía o inválida."); return None
    ...
    except Exception as e_db: logger.error(f"[FAQ] Error al consultar FAQs de BD para rubro {rubro_id}: {e_db}", exc_info=True); return None
    if not faqs: logger.info(f"[FAQ] No se encontraron FAQs en BD para rubro ID {rubro_id}."); return None
    pregunta_limpia = limpiar_texto_base(pregunta_usuario); doc_user = NLP_SPACY_FAQ(pregunta_limpia)
    if not doc_user.has_vector or not doc_user.vector_norm: logger.warning(f"[FAQ] No se pudo generar vector para pregunta: '{pregunta_limpia}'."); return None
    ...
    for faq_item in faqs:
        ...
        except Exception as e_sim: logger.error(f"[FAQ] Error calculando similitud para FAQ ID {faq_item.id} ('{faq_item.question[:50]}...'): {e_sim}", exc_info=True); continue
    if mejor_match and mejor_score >= threshold:
        logger.info(f"✅ [FAQ] Match para '{pregunta_limpia}' (Rubro {rubro_id}): FAQ ID {mejor_match.id} ('{mejor_match.question[:50]}...') score {mejor_score:.3f}")
        return mejor_match
    else:
