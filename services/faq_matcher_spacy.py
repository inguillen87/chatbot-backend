...
from .utils import limpiar_texto_base # <--- AÑADIDO
...
def buscar_en_faq_spacy(pregunta_usuario: str, rubro_id: int, threshold: float = 0.80) -> Optional[QA]:
    ...
    if NLP_SPACY_FAQ is None: # ... (resto de la función como te la pasé en @‶gANVneHZLGZv...)
    ...
    if not faqs:
        ...
        return None
    pregunta_limpia = limpiar_texto_base(pregunta_usuario) # AHORA DEBERÍA FUNCIONAR
